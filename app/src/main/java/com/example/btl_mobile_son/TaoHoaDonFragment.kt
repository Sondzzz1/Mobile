...
class TaoHoaDonFragment : Fragment() {
    ...
    private suspend fun tinhTienDichVu(maPhong: Long, thang: Int, nam: Int): Long {
        ...
        android.util.Log.d("TaoHoaDonFragment", "Tìm thấy ${chiSoDienNuoc.size} chỉ số điện nước cho phòng $maPhong tháng $thang/$nam")
        ...
        for (chiSo in chiSoDienNuoc) {
            ...
            tongTienDichVu += tien
            android.util.Log.d("TaoHoaDonFragment", "  ${chiSo.loai}: ${chiSo.chiSoCu} → ${chiSo.chiSoMoi} (${soTieuThu}) x ${chiSo.donGia} = ${tien}đ")
        }
        
        // 2. Tính tiền các dịch vụ khác (từ bảng phong_dich_vu)
        val phongDichVu = dbManager.phongDichVuDao.layTheoPhong(maPhong)
        android.util.Log.d("TaoHoaDonFragment", "Tìm thấy ${phongDichVu.size} dịch vụ khác cho phòng $maPhong")
        ...
        for (pdv in phongDichVu) {
            ...
            if (dichVu != null && dichVu.isActive) {
                ...
                tongTienDichVu += tien
                android.util.Log.d("TaoHoaDonFragment", "  ${dichVu.tenDichVu}: ${donGia}đ (${dichVu.cachTinh}) = ${tien}đ")
            }
        }
        
        android.util.Log.d("TaoHoaDonFragment", "Tổng tiền dịch vụ: ${tongTienDichVu}đ")
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        try {
            ...
            android.util.Log.e("TaoHoaDonFragment", "Error initializing database", e)
        ...
        btnSubmit.setOnClickListener {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                val maHoaDon = dbManager.hoaDonDao.them(hoaDon)
                
                // Lưu chi tiết từng dịch vụ vào bảng chi_tiet_hoa_don
                if (maHoaDon > 0) {
                    // 1. Thêm chi tiết điện nước
                    val chiSoDienNuoc = dbManager.chiSoDienNuocDao.layTheoThangNam(thangChon, namChon)
                        .filter { it.maPhong == phong?.maPhong }
                    
                    for (chiSo in chiSoDienNuoc) {
                        val soTieuThu = if (chiSo.soTieuThu > 0) chiSo.soTieuThu else (chiSo.chiSoMoi - chiSo.chiSoCu)
                        val thanhTien = soTieuThu * chiSo.donGia
                        
                        val chiTiet = ChiTietHoaDon(
                            maHoaDon = maHoaDon,
                            tenDichVu = if (chiSo.loai == "dien") "Tiền điện" else "Tiền nước",
                            soLuong = soTieuThu.toDouble(),
                            donGia = chiSo.donGia.toDouble(),
                            thanhTien = thanhTien.toDouble()
                        )
                        dbManager.chiTietHoaDonDao.them(chiTiet)
                    }
                    
                    // 2. Thêm chi tiết các dịch vụ khác
                    val phongDichVu = dbManager.phongDichVuDao.layTheoPhong(phong?.maPhong ?: 0)
                    ...
                    for (pdv in phongDichVu) {
                        ...
                        if (dichVu != null && dichVu.isActive) {
                            ...
                            val (soLuong, thanhTien) = when (dichVu.cachTinh) {
                                "theo_phong", "theo_thang" -> Pair(1.0, donGia.toDouble())
                                "theo_nguoi" -> {
                                    val hopDongHienTai = dbManager.hopDongDao.layHopDongDangThue(phong?.maPhong ?: 0)
                                    val soNguoi = hopDongHienTai?.let { 
                                        dbManager.hopDongThanhVienDao.layNguoiDangOTheoHopDong(it.maHopDong).size 
                                    } ?: 1
                                    Pair(soNguoi.toDouble(), (donGia * soNguoi).toDouble())
                                }
                                "mot_lan" -> Pair(0.0, 0.0) // Không tính vào hóa đơn hàng tháng
                                else -> Pair(1.0, donGia.toDouble())
                            }
